...
外部投稿は記事の主たる根拠にせず、購入品レビューやメーカー一次情報を補足する用途に限る。通常は読者が表示操作をした後に外部サービスを読み込む。記事データで `autoDisplay` を明示したYouTubeとRedditの公式埋め込みは、初期表示から投稿を表示する。記事では外部コンテンツの注意文や個別の非表示ボタンを重ねない。自動表示は第三者サービスへ接続し、閲覧者のIPアドレス等が送信される可能性を伴うため、表示する投稿は実利用の参考情報に限り、公式埋め込み元を使う。
...
| Reddit    | 採用   | Reddit公式embed.reddit.com iframe | 公開投稿URLから公式埋め込みURLを生成し、`autoDisplay` 指定時は初期表示する |
...
- 初期HTMLへの第三者scriptは出力しない。
- `autoDisplay` を指定したYouTubeとRedditは、公式iframeで初期HTMLから表示する。これら以外は読者操作後に読み込む。
- Redditの初期表示iframeは、`data-server-embed="true"` と `data-provider="reddit"` を持つ埋め込み枠内にあり、`https://embed.reddit.com/` のURLであることを品質チェックで検証する。YouTubeも同様に公式ホストのみ許可する。
- 元投稿への通常リンクを常に残す。
